...
st.markdown(
    "EDA interactivo y robusto con limpieza automática de datos."
)
...
if uploaded_file:
    ...
    if numeric_cols:
        ...
        st.info(f"🔎 Outliers detectados: {outliers.shape[0]}")
    ...
    if cat_cols:
        cat_col = st.selectbox("Variable categórica", cat_cols)
    ...
    if len(numeric_cols) >= 2:
        ...
        use_trendline = st.checkbox("Agregar línea de tendencia (OLS)", value=False)
        ...
        # Trendline solo si el usuario lo pide
        if use_trendline:
            fig_scatter = px.scatter(
